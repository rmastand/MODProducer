...
for event in content:
	...
	by_lumi[event[2]] = {}
	by_lumi[event[2]]["files present"] = []
	by_lumi[event[2]]["triggers"] = {}
	by_lumi[event[2]]["good_events"] = 0
	for trigger in range(4,len(event),6):
		by_lumi[event[2]]["triggers"][event[trigger]]={"present":0,"fired":0,"prescale1":[],"prescale2":[]}

for event in content:
	by_file[event[3]]["good_events"] += 1
	if event[2] not in by_file[event[3]]["lumis present"]:
		by_file[event[3]]["lumis present"].append(event[2])
	for trigger in range(4,len(event),6):
	...
	by_lumi[event[2]]["good_events"] += 1
	if event[3] not in by_lumi[event[2]]["files present"]:
		by_lumi[event[2]]["files present"].append(event[3])
	for trigger in range(4,len(event),6):
		by_lumi[event[2]]["triggers"][event[trigger]]["present"] +=1
		if [event[trigger+3]]=="true":
			by_lumi[event[2]]["triggers"][event[trigger]]["fired"] +=1
		by_lumi[event[2]]["triggers"][event[trigger]]["prescale1"].append(float(event[trigger+2]))
		by_lumi[event[2]]["triggers"][event[trigger]]["prescale2"].append(float(event[trigger+3]))		
