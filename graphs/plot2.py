...
def graph_eff_lumin():
	...
	for trig_index,trig in enumerate(rev_ordered_triggers):
		...
		eff_lumin = np.array([float(x) for x in lines[2*trig_index+4].split(",")[:-1]])
...
def graph_eff_lumin_time_ordered():
	...
	trig_name_positions = {"HLT_Jet30":(x,100),"HLT_Jet60":(x,4000),"HLT_Jet80":(x,10000),
			      ...
			      "HLT_Jet240":(x,10000000),"HLT_Jet300":(x,50000000),"HLT_Jet370":(x,100000000)}
...
def graph_fired_over_eff_lumin():
	...
	ax.add_artist(logo_box())
        plt.text(-25000,35000,"1223 of 1223 AOD Files",weight="normal")
...
def graph_fired_over_eff_lumin_time_ordered():
	...
	ax.set_xlim(left = 1300086884*.998,right = max(times)*1.0001)
	ax.set_xticklabels(labels)
