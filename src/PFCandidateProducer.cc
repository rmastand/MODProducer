...
int chargedGenVertex( const reco::VertexCollection& vertices,const reco::GenParticle& pfcand  )  {
...
void PFCandidateProducer::produce(Event& iEvent, const EventSetup& iSetup) {
   ...
   if (completedEvents_.find(to_string(runNum) + "_" + to_string(eventNum)) == completedEvents_.end()) {
     try {
	  ...
	  if (dataType_=="Sim"){
		  ostringstream os;
		  os << "#   Hard" << "              px              py              pz          energy           pdgId          status" << endl;   
		  for(reco::GenParticleCollection::const_iterator it = genParticles->begin(), end = genParticles->end(); it != end; it++) {
		    if (it == genParticles->begin())
				...
				if ( status==1 ) {
						...
						energy = it->energy();
					  	const reco::VertexCollection& vertices = *primaryVerticesHandle;
    						int PV = chargedGenVertex(vertices,*it);
