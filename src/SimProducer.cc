...
class SimProducer : public EDProducer 
{
public: 
   explicit SimProducer(const ParameterSet&);
   ~SimProducer();
...
SimProducer::SimProducer(const ParameterSet& iConfig)
: hltConfig_(),
...
std::string SimProducer::to_string ( int number ) {
...
void SimProducer::produce(Event& iEvent, const EventSetup& iSetup) {
...
void SimProducer::beginRun(edm::Run & iRun, edm::EventSetup const & iSetup){
...
void SimProducer::endRun(edm::Run&, edm::EventSetup const&) {

}

void SimProducer::beginLuminosityBlock(edm::LuminosityBlock& iLumi, edm::EventSetup const& iSetup) {
   
}

void SimProducer::endLuminosityBlock(edm::LuminosityBlock& iLumi, edm::EventSetup const& iSetup) {

}

bool SimProducer::triggerFired(const std::string& triggerWildCard, const edm::TriggerResults& triggerResults) {
   bool fired = false;
...
unsigned int SimProducer::findTrigger(const std::string& triggerWildCard) {
...
bool SimProducer::file_exists(const std::string& name) {
  struct stat buffer;   
  return (stat (name.c_str(), &buffer) == 0); 
}

DEFINE_FWK_MODULE(SimProducer);
